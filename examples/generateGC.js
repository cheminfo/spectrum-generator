...
const options = { start: 0, end: xMax, pointsPerUnit: 100, noise: { percent: 2 }, peakWidthFct: (x) => peakWidth };
...
for (let i = 0; i < spectrum.x.length; i++) {
  text.push(`${String(spectrum.x[i].toFixed(2)).padStart(7)} ${String(spectrum.y[i].toFixed(4)).padStart(10)}`);
}
